...
export function activate(ctx) {
    ctx.subscriptions.push(ctx.composerActions.register('git-branch', {
        /**
        ...
        async getBadge({ cwd }) {
            ...
            const { stdout } = await execFileAsync('git', ['-C', cwd, 'branch', '--show-current'], { timeout: 3000 });
        ...
        async getMenu({ cwd }) {
            if (!cwd)
                return [];
            try {
                const { stdout } = await execFileAsync('git', ['-C', cwd, 'branch'], { timeout: 5000 });
                const items = [];
                for (const line of stdout.split('\n').filter(Boolean)) {
                    const current = line.startsWith('* ');
                    const name = line.replace(/^\*?\s+/, '').trim();
                    if (name)
                        items.push({ id: name, label: name, current });
                }
                return items;
            }
            catch {
        ...
        async execute({ cwd }, itemId) {
            if (!cwd || !itemId || itemId === '__error__')
                return;
